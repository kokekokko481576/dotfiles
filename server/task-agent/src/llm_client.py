...
def _load_major_events() -> str:
    ...
    for i in range(2): # 現在月と前月
        target_date = today - timedelta(days=30 * i)
        target_filenames.add(f"{target_date.year}_{target_date.month:02d}_major_events.md")
...
def generate_daily_plan(github_items: list[dict], calendar_events: list[dict]) -> dict:
    ...
    戻り値: {"title": str, "recommendations": [{"task": str, "reason": str}]}
    """
    # 1. GitHub issueを評価 (既存の仕組みを流用)
    scored_issues = []
    for item in github_items:
        try:
            score = _score_issue(item)
            scored_issues.append({**item, **score})
        except Exception:
            log.exception("issue %s#%s の評価に失敗、スキップ", item["repo"], item["number"])
            continue
    scored_issues.sort(key=lambda s: s.get("urgency", 0), reverse=True)

    # (将来的にカレンダーの予定も個別に評価するロジックを追加)

    # 2. 評価済みリストをテキスト化
    scored_issues_text = "\n".join(
        f"- [GitHub#{s['number']}] {s['title']} (urgency={s.get('urgency', '?')}, reason: {s.get('reason', '')})"
        for s in scored_issues
    )
    calendar_events_text = "\n".join(f"- [カレンダー] {e['summary']} ({e['start']} - {e['end']})" for e in calendar_events)


    major_events_block = f"""【過去の主要イベントログ（直近の文脈）】
{MAJOR_EVENTS_TEXT}
...
""" if PROFILE_TEXT else ""
    prompt = f"""{profile_block}{major_events_block}あなたは個人のタスク管理アシスタント「ワニ博士」です。
以下の情報とあなた自身のプロフィールを元に、今日の作戦会議の推奨タスクを生成してください。

【今日のカレンダーの予定】
{calendar_events_text or "なし"}

【今日の関連タスク候補 (GitHub issue)】
{scored_issues_text or "なし"}

上記の情報を踏まえ、今日1日で集中すべきこと、取り組むべきタスクを3〜5個程度提案してください。
提案は、なぜ今日それをやるべきなのかの理由も添えて、簡潔にお願いします。
特にプロフィールに記載の繁忙期や締切を考慮し、現実的な計画を立ててください。

以下のJSON形式のみで出力してください（他のテキストは含めない）:
{{
  "title": "今日の作戦会議",
  "recommendations": [
    {{"task": "推奨タスク1の名前", "reason": "なぜ今日やるべきかの短い理由"}},
    {{"task": "推奨タスク2の名前", "reason": "理由"}},
    {{"task": "推奨タスク3の名前", "reason": "理由"}}
  ]
}}
"""
    return _chat_json(prompt)
