...
def generate_daily_plan(github_items: list[dict], calendar_events: list[dict]) -> dict:
    """
    GitHub issue・カレンダー・進捗ステート(projects)・生活リズムを元に、その日の計画を作る。
...
""" if projects_text else ""
    ...
    prompt = f"""{_context_blocks()}{projects_block}あなたは個人のタスク管理アシスタント「ワニ博士」です。
...
C) 時間割(proposed_schedule): 起床(wake_time)から就寝までを、下の固定予定と食事を避け、
   空き時間にA/Bのタスクを配置する。生活リズムの「集中できる時間帯」を守る(午前=重い等)。
   詰め込みすぎず休憩を挟む。各項目は start/end(HH:MM)。
