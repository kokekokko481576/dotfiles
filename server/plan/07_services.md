...
| 主な環境変数 | `DISCORD_TOKEN`, `DISCORD_GUILD_ID`, `DISCORD_CHANNEL_NOTIFY`, `DISCORD_CHANNEL_CHAT`, `LITELLM_MASTER_KEY`, `LLM_MODEL`, `DISCORD_OWNER_ID`（省略可・エージェントモード用）。全一覧は`.env.example`参照 |
| 起動 | `profiles: [butler]`のため`docker compose --profile butler up -d`（下記「起動・管理コマンド」参照）|
| コマンド | `!status` `!health` `!clear`、`#butler-chat`でのメンション/自然文会話 |
