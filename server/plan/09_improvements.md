...
| SSHのUFW許可範囲 | セキュリティレビューで判明：現状`22/tcp`が`tailscale0`限定ではなく全インターフェースに開いている（`plan/08_security.md`の「外部への直接ポート開放はしない」方針と不整合）。LAN直SSHが不要なら`tailscale0`限定に絞る | P0（対応中、実際のLANサブネットを確認してから調整） |
