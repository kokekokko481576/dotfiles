...
1. **ぼうけん**(デフォルト) — DQウォークのストーリーモード風の横スクロール一枚絵。
   左にワニ博士、アクティブなタスク=道中の敵モンスター(タスクIDのハッシュで
   種類が安定決定: タスクスライム/しめきりコウモリ/さきのばしダケ/みえないおばけ/
   おもごしゴーレム)。HPバーが状態を表す(Todo満タン→In Progress 2/3→Review 1/3)。
   敵タップ→タスクシートで状態変更。完了=とどめ(スロット演出、3割でかいしんのいちげき)
   →討伐→コイン/薬草ドロップ→前進。着手=ツボ割り演出。夜(0-7時)は焚き火で野営。
   待ち/後回しは道の脇(⛺/🧰)に折りたたみ。
2. **リスト** — 初代のカード型UI。
