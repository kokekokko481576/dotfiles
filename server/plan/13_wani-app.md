...
1. **ぼうけん**(デフォルト) — 縁日の射的/アーケード筐体風の「紙人形」ステージ
   ...
   - アクティブなタスク=敵の隊列。**並び順はGitHub Projectの手動順**で、タスクシートの
     「せんとう/まえへ/うしろへ」で変更できる(updateProjectV2ItemPosition、
     GitHub側のボードにも反映)。画面外の敵は「→あと N体」表示。
   - HPバーが状態(Todo満タン→In Progress 2/3→Review 1/3)。完了=とどめ(スロット演出、
     3割でかいしんのいちげき)→討伐→ドロップ→前進。着手=ツボ割り。夜(0-7時)は野営。
   - **Googleカレンダー連携**: 予定の時間になると「じかんまじん」(時計モンスター)が
     最前列に割り込む。タップ→「すんだ！」で解散(localStorageに記録、その日限り)。
     今後の予定は脇道📅に一覧。予定はwani-apiがn8n Webhook経由で取得(5分キャッシュ)。
   待ち/後回しは道の脇(⛺/🧰)に折りたたみ。
2. **リスト** — 初代のカード型UI。
3. **マップ** — すごろく型の俯瞰マップ。討伐済みは旗、現在地にワニ博士、最後に🏰。
