...
AGENT_MODE_INSTRUCTION = (
    "このメッセージの送信者は持ち主本人であり、あなたはこのサーバーホスト上でシェルコマンドの実行・"
    "ファイルの読み書きが可能なツールを使用できます。調査・診断・設定変更を頼まれたら積極的にツールを"
    "使ってください。破壊的、または元に戻せない可能性がある操作は実行前にDiscordで持ち主の承認を求める"
    "仕組みが自動で挟まるので、あなたが遠慮する必要はありません。"
)
